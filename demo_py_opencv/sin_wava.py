...
def cos_wave_surplus_region(initial_phase=0, Period=70,  path="./cos_pictures_grayCode_16/"):
    """
    """
    global Image_cols
    Image_cols_Origin = Image_cols
    Image_cols = Image_cols // Period * (Period)
    ...
    if Image_cols_Origin != Image_cols:
        cos_wave = np.concatenate((cos_wave, img_surplus_region), axis=1)
    ...
    cv2.imwrite(
        path + "cos_wave" + "_Period_" + str(Period) + "_initial_phase_" + str(initial_phase) + ".bmp",
        cos_wave)
...
if __name__ == "__main__":
    ...
    # cos_wave_surplus_region(initial_phase=0, Period=70)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=70)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=70)
    #
    # cos_wave_surplus_region(initial_phase=0, Period=64)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=64)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=64)
    #
    #
    # cos_wave_surplus_region(initial_phase=0, Period=59)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=59)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=59)
    #
    # cos_wave_surplus_region(initial_phase=0, Period=1)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=1)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=1)
    #
    # cos_wave_surplus_region(initial_phase=0, Period=5)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=5)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=5)
    #
    # cos_wave_surplus_region(initial_phase=0, Period=6)
    # cos_wave_surplus_region(initial_phase=-np.pi * 2 / 3, Period=6)
    # cos_wave_surplus_region(initial_phase=np.pi * 2 / 3, Period=6)

    cos_wave_surplus_region(initial_phase=0, Period=16, path="./cos_pictures_grayCode_16/")
    cos_wave_surplus_region(initial_phase=np.pi*1/2, Period=16,path="./cos_pictures_grayCode_16/")
    cos_wave_surplus_region(initial_phase=np.pi*2/2, Period=16,path="./cos_pictures_grayCode_16/")
    cos_wave_surplus_region(initial_phase=np.pi*3/2, Period=16,path="./cos_pictures_grayCode_16/")
