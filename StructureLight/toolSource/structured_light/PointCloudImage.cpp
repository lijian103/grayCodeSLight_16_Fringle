...
bool PointCloudImage::addPoint(int i_w, int j_h, cv::Point3f point)
{
    ...
    cv::Point3f p = (cv::Point3f)Utilities::matGet3D(points,i_w,j_h);
...
void PointCloudImage::exportXYZ(const char *path, bool exportOffPixels, bool colorFlag)
{
...
void PointCloudImage::exportNumOfPointsPerPixelImg(const char *path)
{
	
    cv::Mat projToCamRays(cv::Size(w, h), CV_8U);
	...
	for(int i=0; i<w; i++)
	{
		for(int j=0; j<h; j++)
		{
			...
			Utilities::matSet2D(projToCamRays,i,j, num/max*255.0);
